class CodeLockServerRPC : PluginBase {
    ...
    void ServerRPCHandler(PlayerIdentity sender, Object target, int rpc_type, ParamsReadContext ctx) {
        ...
        switch (rpc_type) {
            ...
            case CLRPC.RPC_SERVER_SYNCHLOCK:
                {
                    Param1<CodeLock> dataCodeLock;
                    if (!ctx.Read(dataCodeLock)) { return; }

                    player = PlayerBase.Cast(target);

                    if (!player) { return; }

                    codelock = dataCodeLock.param1;

                    GetCodeLockLogger().WriteLog("synch", player.GetIdentity(), codelock.GetPosition());

                    SendSynchCodeLockRPC(codelock);
                    break;
                }
        }
    }
                ...
                case "btnRemove":
                    {
                        ...
                        codelock.UnlockServer(player, target);
                        break;
                    }
                case "btnChangePin":
                    {
    ...
    private void SendSynchCodeLockRPC(CodeLock codelock) {
        auto params = new Param3<CodeLock, string, ref array<string>>(codelock, codelock.GetOwner(), codelock.GetGuests());
        GetGame().RPCSingleParam(player, CLRPC.RPC_CLIENT_SYNCHLOCK, params, true, player.GetIdentity());
    }
}
