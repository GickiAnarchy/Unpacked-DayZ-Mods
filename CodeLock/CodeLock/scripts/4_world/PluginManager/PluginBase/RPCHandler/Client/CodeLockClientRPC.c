    ...
    void ClientRPCHandler(PlayerIdentity sender, Object target, int rpc_type, ParamsReadContext ctx) {
        ...
        switch (rpc_type) {
            case CLRPC.RPC_CLIENT_SYNCHLOCK:
                {
                    Param3<CodeLock, string, ref array<string>> dataSynchLock;
                    if (!ctx.Read(dataSynchLock)) { return; }

                    codelock = dataSynchLock.param1;
                    string ownerId = dataSynchLock.param2;
                    ref array<string> arrayGuests = dataSynchLock.param3;
